...
function createLatestPointsUpdateChart() {
    // Assuming the league data is stored in the global variable 'leagueData'
    if (!leagueData || leagueData.length === 0) return;

    // Get the latest and previous data points for each team
    const latestData = leagueData[0];
    const previousData = leagueData[1] || { scores: [] };
